...
const TableRawDataDBD = ({ data, rowsPerPage, monthYear }: Props) => {
  ...
  const sortData = (field: string, direction: 'asc' | 'desc') => {
    return [...data].sort((a, b) => {
      const valueA = a[field as keyof DBDRawDataItem] ?? 0; // Default to 0 if null or undefined
      const valueB = b[field as keyof DBDRawDataItem] ?? 0;
  ...
  const filterData = (dataToFilter: DBDRawDataItem[], term: string) => {
  ...
  const columnSets = {
    basic: ['province', 'city', 'dbd_p', 'dbd_m', 'status'],
    cases: ['province', 'city', 'dbd_p', 'dbd_p_m_to_m_change', 'dbd_p_y_on_y_change', 'status'],
    deaths: ['province', 'city', 'dbd_m', 'dbd_m_m_to_m_change', 'dbd_m_y_on_y_change', 'status'],
    all: ['province', 'city', 'dbd_p', 'dbd_m', 'dbd_p_m_to_m_change', 'dbd_m_m_to_m_change', 'dbd_p_y_on_y_change', 'dbd_m_y_on_y_change', 'status'],
    codes: ['kd_prov', 'kd_kab', 'province', 'city', 'dbd_p', 'dbd_m', 'status']
  };
  ...
  return (
    <Paper withBorder p="md" radius="md">
      <Group justify="space-between" align="flex-end" gap={0} mb="md">
        <Text fz="xl" fw={700}>Tabel Data DBD (Demam Berdarah Dengue), {monthYear}</Text>
        <Group>
          <ActionIcon
            variant="light"
            color="red"
                  ...
                  <Table.Tr key={`${item.kd_prov}-${item.kd_kab}-${item.year}-${item.month}-${index}`}>
                    {visibleColumns.map((column) => (
                      <Table.Td
                        ...
                        style={{
                          color: getCellColor(item[column as keyof DBDRawDataItem], column),
                      ...
                      >
                        {column === 'status' ? (
                          ...
                          formatValue(item[column as keyof DBDRawDataItem], column)
                        )}
                      </Table.Td>
                    ))}
                  </Table.Tr>
...
export default TableRawDataDBD;