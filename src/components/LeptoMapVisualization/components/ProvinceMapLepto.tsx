...
interface ProvinceMapLeptoProps {
  aggregatedData: AggregatedLeptoGeoData[];
  metricToShow: string;
  isActive: boolean;
  popupRef: React.MutableRefObject<Popup>;
}

const ProvinceMapLepto: React.FC<ProvinceMapLeptoProps> = ({ aggregatedData, metricToShow, isActive, popupRef }) => {
  ...
  useEffect(() => {
    ...
    const setupLayers = () => {
      try {
        ...
        console.error('Error setting up province layers for Lepto:', error);
  ...
  const setupTooltipInteraction = useCallback((codeProperty: string, nameProperty: string) => {
    ...
    // In the setupTooltipInteraction function
    map.current.on('mousemove', 'province-fills', (e) => {
      if (e.features && e.features.length > 0) {
        ...
        if (hoveredProvinceId !== provinceCode) {
          ...
          // Update only tooltip position if still in the same province
          // but mouse moves, without changing content
  ...
  const updateTooltipContent = useCallback(
    _.debounce((lngLat: mapboxgl.LngLat, provinceName: string, provinceData: any) => {
      const metricLabel = metricOptionsLepto.find(m => m.value === metricToShow)?.label || 'Nilai';
      ...
      const cfr = provinceData.lep_k > 0 
        ? ((provinceData.lep_m / provinceData.lep_k) * 100).toFixed(2) 
        : 0;
...
// Make sure this useEffect has the proper dependencies
  useEffect(() => {
    if (currentTooltipData) {
      updateTooltipContent(
        currentTooltipData.lngLat,
  ...
  const updateProvinceColors = useCallback(() => {
    ...
    try {
      ...
      console.error('Error updating province colors for Lepto:', error);
    }
  ...
  useEffect(() => {
    if (currentTooltipData && map.current) {
      ...
      // Calculate Case Fatality Rate (CFR)
      const cfr = currentTooltipData.provinceData.lep_k > 0 
        ? ((currentTooltipData.provinceData.lep_m / currentTooltipData.provinceData.lep_k) * 100).toFixed(2) 
        : 0;
...
export default ProvinceMapLepto;