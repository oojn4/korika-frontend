...
const CityMapLepto: React.FC<CityMapLeptoProps> = ({ aggregatedData, metricToShow, isActive, popupRef }) => {
  ...
  const updateTooltipContent = useCallback(
    _.debounce((lngLat: mapboxgl.LngLat, cityName: string, cityData: any) => {
      ...
      const cfr = cityData.lep_k > 0 
        ? ((cityData.lep_m / cityData.lep_k) * 100).toFixed(2) 
        : 0;
  ...
  useEffect(() => {
    if (currentTooltipData && map.current) {
      ...
      // Calculate Case Fatality Rate (CFR)
      const cfr = currentTooltipData.cityData.lep_k > 0 
        ? ((currentTooltipData.cityData.lep_m / currentTooltipData.cityData.lep_k) * 100).toFixed(2) 
        : 0;
