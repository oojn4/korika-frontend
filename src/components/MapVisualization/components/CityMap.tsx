// src/components/MapVisualization/components/CityMap.tsx
...
const CityMap: React.FC<CityMapProps> = ({ aggregatedData, metricToShow, isActive, popupRef }) => {
  ...
  const getMaxValue = useCallback(() => {
    return Math.max(...aggregatedData.map(d => d[metricToShow] || 0), 1);
  ...
  useEffect(() => {
    ...
    const setupLayers = () => {
      try {
        ...
        console.error('Error setting up city layers:', error);
    ...
    map.current.on('mousemove', 'city-fills', (e) => {
      if (e.features && e.features.length > 0) {
        ...
        if (hoveredCityId !== cityCode) {
          ...
          const cityData = aggregatedData.find(d => d.city === cityCode);
        ...
        .setHTML(`
          <div style="background-color: white; color: black; padding: 8px; border-radius: 4px; font-family: sans-serif;">
            ...
            <span style="font-size: 12px; display: block;">
              <strong>Jumlah Faskes:</strong> ${cityData.facility_count}
            </span>
          </div>
        `)
        .addTo(map.current!);
  ...
  useEffect(() => {
    if (currentTooltipData && map.current) {
      // Jangan buat tooltip baru, gunakan yang sudah ada
      const metricLabel = metricOptions.find(m => m.value === metricToShow)?.label || 'Nilai';
      ...
      popupRef.current
        .setHTML(`
          <div style="background-color: white; color: black; padding: 8px; border-radius: 4px; font-family: sans-serif;">
            ...
            <span style="font-size: 12px; display: block;">
              <strong>Jumlah Faskes:</strong> ${currentTooltipData.cityData.facility_count}
            </span>
          </div>
        `);
      // Tidak perlu memanggil addTo() lagi, karena popup sudah ada
    }
