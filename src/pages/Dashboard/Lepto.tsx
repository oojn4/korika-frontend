...
const LeptoPage = () => {
  const [aggregateData, setAggregateData] = useState<LeptoAggregateDataItem[]>([]);
  const [mergedData, setMergedData] = useState<any[]>([]);
  const [filteredData, setFilteredData] = useState<any[]>([]);
  const [rawData, setRawData] = useState<LeptoRawDataItem[]>([]);
  ...
  const handleFetchAggregateData = async () => {
    try {
      ...
      const response = await DashboardService.indexAggregateDataLepto(
        province || '00',
        city || '',
        start,
        end
  ...
  const updateKPIData = (data: LeptoAggregateDataItem[]) => {
    ...
    // Calculate KPIs
    const totalCases = latestData.lep_k || 0;
    const totalDeaths = latestData.lep_m || 0;
    
    // Case fatality rate
    const caseFatalityRate = totalCases > 0 ? (totalDeaths / totalCases) * 100 : 0;
    
    // Monthly change rates
    const totalRate = latestData.lep_k_m_to_m_change || 0;
    const deathRate = latestData.lep_m_m_to_m_change || 0;
    
    // Year on year changes
    const yoyChangeCase = latestData.lep_k_y_on_y_change || 0;
    const yoyChangeDeath = latestData.lep_m_y_on_y_change || 0;
  ...
  const handleFetchRawData = async () => {
    try {
      ...
      const response = await DashboardService.indexRawDataLepto(
        provinceParam,
        city || '',
        monthYear
      );
  ...
  useEffect(() => {
    if (aggregateData?.length) {
      ...
      const maxActualItem = actualData.reduce<LeptoAggregateDataItem | null>((maxItem, currentItem) => {
        if (
          maxItem === null ||
        ...
        if (actualItem) {
          Object.keys(actualItem).forEach(key => {
            if (key !== 'monthYear') {
              ...
              result[`actual_${key}`] = actualItem[key as keyof LeptoAggregateDataItem];
            }
          });
        }
        ...
        if (predictedItem) {
          Object.keys(predictedItem).forEach(key => {
            if (key !== 'monthYear') {
              ...
              result[`predicted_${key}`] = predictedItem[key as keyof LeptoAggregateDataItem];
            }
          });
        }
      ...
      if (maxActualItem) {
        ...
        const mappedData = nextMonthPredictedData.map((predictedItem) => {
          return Object.keys(predictedItem)
            .filter((key) => typeof predictedItem[key as keyof LeptoAggregateDataItem] === 'number')
            .map((attribute) => {
              const yonyKey = `${attribute}_y_on_y_change`;
              const mtomKey = `${attribute}_m_to_m_change`;
              return {
                label: `predicted_${attribute}`,
                count: typeof predictedItem[attribute as keyof LeptoAggregateDataItem] === 'number' 
                  ? predictedItem[attribute as keyof LeptoAggregateDataItem] as number
                  : 0,
                y_on_y: typeof predictedItem[yonyKey as keyof LeptoAggregateDataItem] === 'number' 
                  ? (predictedItem[yonyKey as keyof LeptoAggregateDataItem] as number) 
                  : null,
                m_to_m: typeof predictedItem[mtomKey as keyof LeptoAggregateDataItem] === 'number' 
                  ? (predictedItem[mtomKey as keyof LeptoAggregateDataItem] as number) 
                  : null,
                color: (Number(predictedItem[attribute as keyof LeptoAggregateDataItem]) ?? 0) >= 0 ? 'green' : 'red',
                color_y_on_y: (Number(predictedItem[yonyKey as keyof LeptoAggregateDataItem]) ?? 0) <= 0 ? 'green' : 'red',
                color_m_to_m: (Number(predictedItem[mtomKey as keyof LeptoAggregateDataItem]) ?? 0) <= 0 ? 'green' : 'red',
              };
            });
        });
    ...
    return (
      <>
        ...
        <SimpleGrid cols={{ base: 1, xs: 2, md: 2 }}>
          <Box>
            <Statbox
              title="Kasus Lepto (Lepto_P)"
              icon={IconArticle}
              data={getChartData()}
              textStatbox={textStatbox}
              dataKey="monthYear"
              series={[
                { name: 'predicted_lep_k', color: 'grey' },
                { name: 'actual_lep_k', color: 'red.6' },
              ]}
              isCollapsible
            />
          </Box>
          ...
          <Box>
            <Statbox
              title="Kematian Lepto (Lepto_M)"
              icon={IconArticle}
              data={getChartData()}
              textStatbox={textStatbox}
              dataKey="monthYear"
              series={[
                { name: 'predicted_lep_m', color: 'grey' },
                { name: 'actual_lep_m', color: 'red.9' },
              ]}
              isCollapsible
            />
          </Box>
